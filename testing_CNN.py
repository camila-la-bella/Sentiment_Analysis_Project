...
# CNN模型类
class TextSent_CNN(nn.Module):
    def __init__(self, embedding_matrix, filter_sizes=(2, 3, 4), num_filters=64, dropout=0.3):
        super().__init__()
    ...
    def forward(self, x):
        ...
        conv_outs = [F.adaptive_max_pool1d(F.relu(conv(x)), 1).squeeze(-1) for conv in self.convs]
...
def evaluate_metrics(model, dataloader):
    ...
    acc = accuracy_score(all_labels, all_preds) #计算accuracy
...
if __name__ == "__main__":
    ...
    model = TextSent_CNN(embedding_matrix, filter_sizes=(2, 3, 4), num_filters=64, dropout=0.3).to(DEVICE)
